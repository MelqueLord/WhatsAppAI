...
- [X] **T003** Configurar PostgreSQL 17 no Docker Compose e EF Core/Npgsql, sem criar migration. **Refs:** NFR-007, NFR-009, R-003, ADR-0005, ADR-0008. **Paths:** `compose.yaml`, `src/WhatsAppAI.Infrastructure/`, configuração local da WebApi. **Depends:** T001. **Aceite:** PostgreSQL sobe, readiness passa e DbContext conecta sem schema de negócio/migration antecipada.
...
- [X] **T044** Integrar compositor com modo e bloqueio/explicação de janela fechada. **Refs:** US-003, FR-011, FR-012, BR-003, BR-006. **Paths:** `apps/web/src/features/inbox/composer/`, testes frontend. **Depends:** T032, T040, T042. **Aceite:** operador assume antes de enviar; texto fora da janela não é submetido e o formulário de template fica disponível somente para API Oficial.
...
- Templates de marketing, mensagens proativas e templates utilitários ainda não transacionais (**BR-006**, **R-009**).
...
| ~~G8~~ | ~~**Sem template messages**~~ | ~~Comunicação pós-24h impossível~~ | Resolvido (T203; somente API Oficial) |
...
- [X] **T119** Corrigir CI para usar PostgreSQL. **Refs:** G2, T005.
...
- [X] **T184** Integrar circuit breaker isolado por tenant/provedor e retry limitado do worker, finalizando mensagens com fallback/handoff quando o circuito estiver aberto ou as tentativas se esgotarem. **Refs:** FR-050, BR-AI-007, BR-009, NFR-009. **Paths:** `CircuitBreaker`, `AiOrchestrationWorker` e testes unitários. **Depends:** T183. **Aceite:** após o limiar de falhas novas chamadas são bloqueadas; recuperação permite uma sonda; cada mensagem termina processada e não entra em loop infinito.
- [X] **T185** Tornar atômicas as alterações de configuração/modo/handoff com auditoria e validar `If-Match` antes das escritas, incluindo o registro de `HandoffEvent` na mesma transação. **Refs:** FR-051, FR-011, FR-019, NFR-009. **Paths:** endpoints de IA, BOT e modo de conversa, testes de integração. **Depends:** T184. **Aceite:** conflito de versão não persiste nenhuma parte da alteração; sucesso grava configuração, modo, auditoria e handoff juntos.
- [X] **T186** Implementar gate de avaliação aprovada para ativação de modelo, persistir aprovação/rejeição e disponibilizar rollback para modelo seguro com `If-Match`. **Refs:** FR-052, T057, FR-AI-012. **Paths:** `ModelEvaluationRepository`, endpoints de IA, `AiOrchestrationWorker` e testes. **Depends:** T185. **Aceite:** modelo sem avaliação aprovada não ativa nem é usado pelo worker; rollback valida modelo aprovado, versão e registra auditoria.
- [X] **T187** Padronizar provedores e catálogo de modelos em fonte única no Application, alinhando API, frontend, DI e política de modelos com normalização de identificadores e ordem estável. **Refs:** FR-AI-013, FR-AI-001, FR-AI-006, FR-AI-007. **Paths:** `Automation/Policy/AiProviderCatalog`, `AiModelPolicy`, `AiProviderResolver`, `AiProviderEndpoints`, `AiConfigPage` e testes. **Depends:** T186. **Aceite:** nenhum catálogo duplicado permanece; todos os provedores registrados aparecem com os modelos da fonte única; IDs de provedor/modelo são normalizados de forma consistente.
- [X] **T196** Adicionar recargas administrativas mensais e idempotentes de 500 respostas, somá-las à franquia efetiva e suspender automaticamente somente a IA quando o saldo chegar a zero. **Refs:** US-006, US-012, US-013, FR-044, FR-045, FR-047, BR-022. **Paths:** políticas de quota, `UsageLedger`, endpoints/administração, worker, dashboard e testes. **Depends:** T182. **Aceite:** cada ação adiciona exatamente 500 ao mês UTC corrente sem alterar o limite-base; repetição da mesma requisição não duplica crédito; franquia esgotada bloqueia IA e recarga ou novo mês libera automaticamente.
- [X] **T197** Permitir distribuir o total de linhas de cada plano entre API Oficial e QR Code no cadastro e na edição do tenant. **Refs:** US-012, FR-029, FR-030, FR-031, FR-053, BR-021, BR-023, FR-P021, BR-P011. **Paths:** `SubscriptionPlan`, endpoints/administração, tela de empresas, migration e testes. **Depends:** T178. **Aceite:** STAR aceita 1 API ou 1 QR; FLOW aceita qualquer distribuição que some 2; backend rejeita soma diferente do total do plano; alteração preserva isolamento e concorrência otimista.
- [X] **T198** Restringir tokens, provedor, modelo e custo real ao PlatformAdmin e manter no tenant somente a franquia de respostas e alertas. **Refs:** US-006, US-013, FR-046, FR-047, FR-054, BR-024, FR-P019. **Paths:** endpoint de uso do tenant, tela de uso, painel de empresas e testes de autorização/contrato. **Depends:** T182. **Aceite:** `/api/usage` não retorna tokens/custo para TenantOwner/Operator; `/api/admin/tenants/{id}/ai-usage` mantém dados reais por empresa; UI do tenant não apresenta termos ou valores técnicos.
- [X] **T199** Otimizar o orçamento de tokens do contexto e da saída de IA. **Refs:** US-004, US-005, FR-013, FR-055, BR-025. **Paths:** `ContextAssembler`, credencial de IA, worker e testes unitários. **Aceite:** apenas quatro mensagens recentes, três conhecimentos relevantes e conteúdo limitado chegam ao provedor; regras e JSON obrigatório permanecem completos; respostas ficam limitadas a 240 tokens.
- [X] **T200** Priorizar a base de conhecimento para fatos da empresa e bloquear contexto não relacionado. **Refs:** US-004, US-005, FR-013, FR-056, BR-026. **Paths:** `ContextAssembler` e testes unitários. **Aceite:** somente itens com correspondência lexical entram no prompt; ausência de correspondência informa a IA para não inventar e encaminhar fatos empresariais não documentados.
- [X] **T201** Ampliar o perfil estruturado do negócio na tela de IA, reutilizando o campo de diretrizes existente e mantendo a base de conhecimento como fonte de fatos. **Refs:** FR-AI-003, FR-057, BR-027. **Paths:** `AiConfigPage`, teste de interface e documentação. **Aceite:** TenantOwner informa descrição, público, produtos/serviços, tom, horário e localização; os campos são carregados e salvos junto às diretrizes sem novo endpoint paralelo.
- [X] **T202** Implementar expediente configurável do BOT por dia e fuso horário, com bloqueio fora do horário e mensagem offline. **Refs:** US-004, FR-058, BR-028. **Paths:** `BotConfiguration`, `BusinessHoursPolicy`, endpoint/tela do BOT, worker, migration e testes. **Aceite:** agenda habilitada valida sete dias e horários; o worker usa o fuso configurado antes de criar mensagem/outbox; agenda desabilitada preserva operação 24 horas; sem mensagem offline não cria resposta.
- [X] **T203** Implementar templates transacionais aprovados pela Meta somente na API Oficial para conversas fora da janela de 24 horas, com parâmetros limitados, persistência na Outbox e rejeição explícita no QR Code. **Refs:** FR-012, FR-059, BR-006, BR-029, G8. **Paths:** `Message`, `IWhatsAppClient`, adaptadores Meta/QR, `ConversationEndpoints`, `OutboxProcessingWorker`, frontend Inbox, migration e testes. **Depends:** T042, T044. **Aceite:** template com nome/idioma válido é enfileirado e enviado pela Cloud API; texto livre continua bloqueado fora da janela; template em QR Code ou payload inválido é rejeitado sem chamada externa; migration é reversível.
- [X] **T204** Garantir ownership exclusivo e failover seguro para sessões QR entre múltiplas instâncias da ponte. **Refs:** FR-060, BR-030, NFR-009, ADR-0011. **Paths:** persistência PostgreSQL, `WebhookEndpoints`, `WhatsAppWebClient`, `services/whatsapp-web`, Compose, runbook e testes. **Aceite:** apenas o dono com lease vigente abre ou opera a sessão; réplica concorrente recebe o endereço do dono; após expiração, uma única réplica assume sem corrupção de credenciais.
- [X] **T205** Declarar e documentar uma segunda ponte QR isolada para produção, com URL, identidade, volume e healthcheck próprios. **Refs:** FR-060, BR-030, ADR-0011. **Paths:** `compose.yaml`, ambiente de produção e runbook. **Aceite:** o perfil de escala inicia duas pontes sem compartilhar volume; API/worker podem alcançar os dois endereços internos; a configuração não usa `docker compose --scale`.
- [X] **T206** Rejeitar webhooks QR emitidos por uma ponte sem lease vigente e interromper a repetição local após conflito de ownership. **Refs:** FR-060, BR-030, ADR-0011. **Paths:** `WebhookEndpoints`, `services/whatsapp-web` e testes de integração. **Aceite:** a ponte antiga recebe conflito ao publicar evento após perder o lease; nenhum evento dessa ponte é persistido ou reenviado repetidamente.
- [X] **T207** Validar operação simultânea de múltiplos canais QR do mesmo tenant, preservando sessão, credencial, inbox e lease por linha. **Refs:** FR-031, FR-060, BR-030, ADR-0011. **Paths:** `services/whatsapp-web`, `WhatsAppWebClient`, persistência e testes de integração. **Aceite:** duas linhas QR do mesmo tenant podem adquirir leases independentes e não colidem no ownership.

### Fase 11 — IA contratada e administrada pela plataforma

- [X] **T208** Restringir provisionamento, rotação e teste de credenciais de IA ao PlatformAdmin, mantendo diretrizes, perfil, conhecimento, confiança, filas, tags e handoff configuráveis pelo TenantOwner no próprio tenant. **Refs:** FR-004, FR-021, FR-062, BR-008, BR-031, FR-AI-004, FR-AI-005. **Paths:** `AiProviderEndpoints`, `AdminAiProviderEndpoints`, `AiProviderCredential`, `AiConfigPage`, `AdminTenantsPage`, autorização, auditoria e testes. **Depends:** T187, T201. **Aceite:** TenantOwner nunca recebe nem altera API key; PlatformAdmin provisiona/testa a credencial; configurações de atendimento permanecem isoladas no tenant.
- [X] **T209** Modelar escopo da credencial administrada pela plataforma, suportando referência segregada por tenant/projeto ou credencial compartilhada com controle interno. **Refs:** FR-004, FR-AI-004, BR-008. **Paths:** domínio/infraestrutura de integrações, migration reversível, `ISecretStore`, contratos e testes de isolamento. **Depends:** T208. **Aceite:** nenhuma chave é persistida em claro; rotação não expõe segredo; credencial de um tenant não pode ser usada por outro.
- [X] **T210** Implementar orçamento mensal de custo e limite técnico de tokens por tenant, com reserva/revalidação antes da chamada, registro do custo real após a resposta e bloqueio com fallback/handoff ao atingir qualquer limite. **Refs:** FR-046, FR-048, FR-049, FR-061, BR-AI-006. **Paths:** `Tenant`, `UsageLedger`, política de orçamento, `AiOrchestrationWorker`, endpoints administrativos, UI e testes. **Depends:** T183, T208. **Aceite:** chamadas acima do orçamento não são iniciadas; uma concorrência não ultrapassa o orçamento reservado; o bloqueio é tenant-scoped e auditado.
- [X] **T211** Adequar a tela administrativa de uso para custo operacional, orçamento, margem, provedor/modelo e franquia, mantendo a tela `/usage` do tenant limitada a respostas, saldo e alertas. **Refs:** US-006, US-013, FR-047, FR-054, FR-061, BR-024. **Paths:** `AdminTenantsPage`, `UsagePage`, contratos, autorização e testes frontend. **Depends:** T208, T210.
- [X] **T212** Formalizar contrato da franquia de respostas, período mensal UTC, saldo com reservas pendentes e estados `Pending`/`Committed`/`Released`. **Refs:** US-006, FR-044, FR-045, FR-047, BR-AI-006. **Paths:** `AiResponseQuotaContract`, políticas de quota e testes unitários.
- [X] **T213** Persistir reservas de respostas e solicitações de recarga de 500 respostas com isolamento por tenant, idempotência, índices e migration reversível. **Refs:** US-012, US-013, FR-044, FR-045, FR-019. **Paths:** domínio `Usage`, `AppDbContext`, configurações EF, migration e testes unitários. **Depends:** T212.
- [X] **T214** Implementar serviço transacional de quota com reserva atômica por tenant/período, idempotência por mensagem, commit, release e snapshot de saldo. **Refs:** FR-045, BR-AI-006, NFR-009. **Paths:** `IAiResponseQuotaService`, `AiResponseQuotaService`, DI e testes PostgreSQL. **Depends:** T213.
- [X] **T215** Integrar reserva, commit e release de respostas ao `AiOrchestrationWorker`, bloqueando chamadas sem saldo e liberando quota em falha, handoff, resposta inválida ou perda de concorrência. **Refs:** FR-045, FR-061, BR-AI-006, NFR-009. **Paths:** `AiOrchestrationWorker`, `IAiResponseQuotaService` e testes unitários/integrados. **Depends:** T214.
- [X] **T216** Implementar reconciliador de reservas pendentes expiradas, com logs operacionais sanitizados e execução periódica. **Refs:** FR-045, BR-AI-006, NFR-009. **Paths:** `AiResponseQuotaReconciler`, `AiResponseQuotaReconciliationWorker` e testes PostgreSQL. **Depends:** T215.
- [X] **T217** Criar tela administrativa para cadastrar e consultar preços versionados por provedor/modelo, alimentando o cálculo de custo operacional. **Refs:** FR-048, FR-049, US-006, US-013. **Paths:** `AdminAiPricingPage`, navegação, rota, contrato da API e testes frontend. **Depends:** T183.
- [X] **T218** Atribuir tokens, custos e respostas ao pacote-base ou à recarga utilizada pela empresa e exibir o consumo separado por pacote. **Refs:** FR-044, FR-046, FR-047, FR-048, US-006, US-013. **Paths:** reserva de quota, `UsageLedger`, migration, endpoint e painel administrativo. **Depends:** T216, T217.
- [X] **T219** Tornar a franquia de respostas a única trava operacional da IA, avisar o tenant ao atingir 80% e reduzir o contexto/saída para consumo mínimo de tokens. **Refs:** FR-044, FR-045, FR-046, FR-047, FR-055. **Paths:** worker, contexto, políticas de saída e telas de uso. **Depends:** T218.
- [X] **T220** Refinar o uso do perfil estruturado de cada empresa no contexto e na simulação de IA, preservando diretrizes do tenant, conhecimento factual e contexto mínimo. **Refs:** US-004, US-005, FR-013, FR-055, FR-056, FR-057, BR-025, BR-026, BR-027. **Paths:** `ContextAssembler`, diretrizes de IA, simulação e testes. **Depends:** T219.
- [X] **T221** Implementar base de conhecimento guiada por categoria, preservando itens legados, isolamento e seleção factual da IA. **Refs:** US-005, FR-013, FR-017, FR-056, FR-063, BR-026, BR-032. **Paths:** domínio Knowledge, migration, endpoints, tela e testes. **Depends:** T220. **Aceite:** o TenantOwner cadastra pergunta frequente, serviço, preço, horário, pagamento, localização, política ou informação geral com orientação de preenchimento; itens anteriores viram Geral; a categoria é validada, retorna na API e não altera a regra de relevância da IA.
- [X] **T222** Implementar exemplos de atendimento por empresa e selecionar no máximo um exemplo relevante para orientar respostas e simulações. **Refs:** US-004, FR-013, FR-055, FR-062, FR-064, BR-025, BR-031, BR-033. **Paths:** domínio/infraestrutura Automation, migration, contexto, endpoints, tela, navegação e testes. **Depends:** T221. **Aceite:** TenantOwner gerencia exemplos com concorrência otimista; somente exemplo ativo do tenant e relacionado à mensagem entra no contexto; exemplos nunca substituem conhecimento factual; simulação e worker usam a mesma seleção limitada.
- [X] **T223** Implementar testes diagnósticos de IA por cenário usando a configuração vigente do tenant. **Refs:** US-004, FR-013, FR-055, FR-062, FR-063, FR-064, FR-065, BR-025, BR-031, BR-032, BR-033, BR-034. **Paths:** contexto, endpoint de simulação, tela, navegação e testes. **Depends:** T222. **Aceite:** TenantOwner testa cenários predefinidos ou personalizados sem enviar WhatsApp nem consumir franquia; a simulação usa perfil, conhecimento e exemplo relevantes do tenant; resultado exibe decisão, texto, confiança e handoff sanitizados; nenhum conteúdo completo do teste é persistido.
- [X] **T224** Corrigir a navegação lateral em telas móveis, preservando rodapé e controle de borda visíveis para todos os papéis. **Refs:** FR-066. **Paths:** `apps/web/src/components/Sidebar.tsx`, `Layout.tsx`, testes de componente e documentação. **Aceite:** cabeçalho e rodapé não saem da área visível; apenas a lista de itens rola; no desktop o controle recolhe/expande o menu e, no mobile, fecha o drawer que o botão do cabeçalho reabre em largura completa, inclusive para Operator.
- [X] **T225** Criar a finalidade padrão de atendimento automatizado por IA no provisionamento de cada empresa, mantendo consentimento por contato obrigatório. **Refs:** FR-067, BR-035, ADR-0008. **Paths:** `AdminTenantEndpoints`, testes de provisionamento e especificação. **Aceite:** toda empresa nova recebe exatamente uma finalidade ativa, isolada por tenant, com base legal `Consent`; nenhuma mensagem automática é processada sem evidência ativa do contato.
- [X] **T226** Implementar aceite WhatsApp padronizado de consentimento para IA em todas as empresas. **Refs:** FR-067, FR-068, BR-035, BR-036, ADR-0008. **Paths:** política de consentimento, `AiOrchestrationWorker`, Outbox, auditoria, testes e especificação. **Depends:** T225. **Aceite:** contato sem consentimento recebe uma única solicitação; somente `SIM` normalizado cria evidência ativa com referência da mensagem; confirmações e solicitações são idempotentes, não consomem franquia de IA e permanecem isoladas por tenant.
- [X] **T227** Encerrar conversas por ação explícita do operador, exibir a fila de encerradas e reabrir a mesma conversa no automático quando o cliente voltar. **Refs:** US-014, FR-013, FR-024, FR-069, BR-004, BR-037. **Paths:** `Conversation`, `ConversationEndpoints`, consultas da inbox, `WebhookProcessingWorker`, Inbox React, testes unitários e de integração. **Aceite:** `If-Match` protege o encerramento; a conversa fechada sai das abertas e mantém todo o histórico; nova mensagem reabre o mesmo ID, remove atribuição/fila, retorna ao automático e preserva as três mensagens anteriores para o contexto da IA.
- [X] **T228** Persistir memória institucional automática por tenant a partir de respostas de IA seguras e fundamentadas em conhecimento ativo, reutilizando-a na seleção de contexto sem registrar dados pessoais ou cruzar empresas. **Refs:** FR-013, FR-062, FR-070, BR-025, BR-026, BR-031, BR-038. **Paths:** KnowledgeItem, CompanyMemoryPolicy, AiOrchestrationWorker, ContextAssembler, testes unitários e de isolamento. **Depends:** T220, T221. **Aceite:** resposta com confiança mínima de 0,8 e fonte ativa gera memória sanitizada da empresa; resposta sem fonte, handoff ou conteúdo inseguro não gera memória; outro cliente do mesmo tenant pode recuperar a memória e outro tenant não pode.
- [X] **T229** Registrar feedback do operador em respostas de IA e transformar correções seguras em conhecimento validado do tenant, sem novo envio automático. **Refs:** FR-071, BR-039, FR-017, FR-019. **Paths:** `AiInteraction`, migration, `AiFeedbackEndpoints`, consultas da Inbox, `MessagePanel`, testes de domínio e frontend. **Depends:** T228. **Aceite:** resposta da IA exibe ações útil/correção; feedback é tenant-scoped e auditado; correção de até 160 caracteres fica disponível como conhecimento; feedback não dispara WhatsApp.
- [X] **T230** Ampliar a inferência segura do agente por empresa, combinando perguntas gerais, paráfrases, categorias de conhecimento, diretrizes, exemplos e memória sem cruzar tenants nem inventar fatos. **Refs:** FR-072, BR-032, BR-040, FR-013, FR-055, FR-070, FR-071. **Paths:** `ContextAssembler`, políticas de inferência, worker de IA, simulação, testes unitários e documentação. **Depends:** T229. **Aceite:** perguntas como “o que a empresa faz?” e paráfrases recuperam fatos de serviço/gerais compatíveis; exemplos não são tratados como fatos; transferência permanece para lacuna real, pedido humano ou segurança.
- [X] **T231** Ampliar tipos de negócio e tons de conversa e aplicar um guia seguro de segmento ao agente em produção e simulação. **Refs:** FR-073, BR-027, BR-040, BR-041. **Paths:** `AiConfigPage`, `BusinessProfileGuidePolicy`, `ContextAssembler` e testes unitários/frontend. **Depends:** T230. **Aceite:** o TenantOwner escolhe entre segmentos e tons ampliados; o prompt recebe orientação coerente com o segmento; perguntas genéricas recebem resposta útil; fatos específicos não são inferidos do catálogo.
