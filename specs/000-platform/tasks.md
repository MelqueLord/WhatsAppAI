...
- [X] **T022** Implementar `IWhatsAppClient` e teste de conexão Meta sanitizado. **Refs:** US-001, FR-021, BR-008. **Paths:** `src/WhatsAppAI.Application/Integrations/`, `src/WhatsAppAI.Infrastructure/Meta/`, testes de contrato. **Depends:** T020, T021. **Aceite:** sucesso/falha identifica etapa sem revelar credencial e usa somente API oficial.
...
- [X] **T081** Configurar TLS, CORS, headers, rate limits, backup e restore ensaiado. **Refs:** FR-001, FR-005, NFR-005, NFR-008. **Paths:** `deploy/nginx/default.conf`, `deploy/backup.sh`, `deploy/restore.sh`, `docs/runbooks/disaster-recovery.md`.
...
| G6 | **Test coverage baixo** (~159 testes) | Regressões não detectadas | Melhorado |
