...
6. Opcionalmente, o TenantOwner escolhe uma fila ativa do tenant para agrupar os contatos novos no disparo em massa; eles continuam disponíveis na lista geral e contatos preexistentes não são alterados.
...
- **FR-039:** permitir ao TenantOwner importar até 5.000 contatos por arquivo `.csv` ou `.xlsx` de até 2 MB, usando as colunas obrigatórias `nome` e `contato`, com resultado parcial e isolamento pelo tenant corrente. A importação pode associar somente os contatos novos a uma fila ativa do tenant, sem removê-los da lista geral nem modificar duplicados.
...
- **BR-006:** template de Marketing aprovado pode ser enviado manualmente somente na Inbox de uma conversa individual da API Oficial; campanhas em massa de Marketing, Autenticação e componentes não suportados permanecem bloqueados. A categoria e o status informados pela Meta são revalidados pelo backend no envio.
...
- **BR-019:** a importação normaliza `contato` para dígitos em formato internacional de 8 a 15 caracteres, ignora duplicados do arquivo ou já existentes no tenant e nunca atualiza um contato preexistente. Quando selecionada, a fila deve estar ativa e pertencer ao tenant; ela agrupa os novos contatos para seleção no disparo em massa.
