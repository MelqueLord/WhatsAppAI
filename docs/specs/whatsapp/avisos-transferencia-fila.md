...
1. **Given** a conversation remains in an automatic queue, **When** the customer sends another message without a different queue keyword, **Then** the customer receives `Sua conversa segue na fila {nome}. Se quiser mudar de assunto, me diga por aqui.` and the conversation remains automated in the same queue.
...
3. **Given** a new automatic conversation, **When** the first message is processed by the AI, **Then** the configured business-specific welcome message is provided to the AI as the first-contact response guidance; when that message is empty or generic, the AI derives a short welcome from the tenant profile and company name.
