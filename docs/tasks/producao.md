...
> Antes deste checklist, concluir `../design/plano-prontidao-producao.md` e obter `GO` em `gates-producao.md`.
...
> **Revisão posterior (2026-09-13):** o status abaixo registra as validações realizadas naquela data, mas não representa aprovação de segurança para produção. A [avaliação de padrões de indústria](../pesquisa/avaliacao-padroes-industria.md) abriu o incremento T255–T269; este checklist permanece **NO-GO** até seus achados críticos e altos terem evidência de aceite.
...
- [ ] Temporary testing fallback only: `DataProtection__AllowUnencryptedKeys=true` may be used without a PFX; remove it or set it to `false` before production.
