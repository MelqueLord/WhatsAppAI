...
A importação de planilha reutiliza `Contact` e não cria entidade paralela. O arquivo é transitório e não é persistido; cada linha válida é normalizada e submetida à unicidade tenant-scoped, enquanto o relatório retorna apenas número da linha e código de erro. Opcionalmente, `Contact.queue_id` referencia uma fila ativa do mesmo tenant para seleção posterior no disparo em massa; a associação não remove o contato da lista geral nem altera duplicados (**US-010**, **FR-039**, **BR-019**).
