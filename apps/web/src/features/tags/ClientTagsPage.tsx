...
export function ClientTagsPage() {
  ...
  const { data: tags, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetchApiResponse('/api/client-tags')
  ...
  const updateMutation = useMutation({
    mutationFn: async (tag: ClientTag) => {
      const res = await fetchApiResponse(`/api/client-tags/${tag.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ name, color, description }),
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({ id, action }: { id: string; action: 'deactivate' | 'reactivate' }) => {
      const res = await fetchApiResponse(`/api/client-tags/${id}/${action}`, {
        method: 'POST',
        credentials: 'include',
      })
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="flex items-center justify-between gap-3 mb-6 sm:mb-8">
          ...
          >
            <Plus className="w-4 h-4" /> <span className="hidden sm:inline">Nova Tag</span>
          </button>
        </div>
