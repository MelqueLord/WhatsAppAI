...
function UsageRow({ tenant }: { tenant: Tenant }) {
  ...
  return <tr className="hover:bg-slate-50"><td className="px-5 py-4"><div className="font-medium text-slate-800">{tenant.name}</div><div className="text-xs text-slate-500">{tenant.status}</div></td><td className="px-5 py-4"><div className="font-medium text-slate-700">{tenant.aiProvider ?? 'Não configurado'}</div><div className="text-xs text-slate-500">{tenant.aiModelId ?? '—'}</div></td><td className="px-5 py-4"><div>{(tenant.monthlyAiResponsesUsed ?? 0).toLocaleString('pt-BR')} / {responseLimit?.toLocaleString('pt-BR') ?? 'Ilimitado'}</div><div className="mt-1 h-1.5 w-32 rounded-full bg-slate-200"><div className="h-full rounded-full bg-violet-500" style={{ width: `${responsePercent}%` }} /></div></td><td className="px-5 py-4"><div>{(tenant.monthlyAiTokensUsed ?? 0).toLocaleString('pt-BR')}</div><div className="text-xs text-slate-500">Acompanhamento, sem bloqueio</div></td><td className="px-5 py-4"><div>{money(tenant.monthlyAiEstimatedCostMinorUnits ?? 0)}</div><div className="text-xs text-slate-500">Acompanhamento, sem bloqueio</div></td><td className="px-5 py-4 text-xs text-slate-600">A IA é protegida exclusivamente pela franquia de respostas contratada.</td><td className="px-5 py-4">{tenant.isAiSuspendedByQuota ? <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">Bloqueada</span> : <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">Operacional</span>}</td></tr>
}
