...
export function OperatorsPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { email?: string; displayName?: string } }) =>
      api.operators.update(id, data),
    onSuccess: (updated) => {
      queryClient.setQueryData<Operator[]>(['operators'], (current = []) =>
        current.map((op) => (op.id === updated.id ? updated : op))
      )
      setEditTarget(null)
    },
  })
                      ...
                      <tr key={operator.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 sm:px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              onClick={() => setEditTarget(operator)}
                              className="text-xs sm:text-sm text-slate-600 hover:text-slate-800 font-medium whitespace-nowrap"
                            >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-slate-800">Editar Operador</h2>
